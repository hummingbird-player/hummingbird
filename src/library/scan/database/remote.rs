...
use crate::{
    library::{scan::artist_match::ArtistMatcher, source::SourceId},
...
pub(crate) async fn write_remote_batch(
    ...
    for album in albums {
        let album_id = upsert_remote_album(&mut transaction, source, generation, album).await?;
    ...
    let track_id: i64 = sqlx::query_scalar(
        "INSERT INTO track \
         (title, title_sortable, album_id, track_number, disc_number, duration, location, \
          ...
          track_section, number_display_mode_hint, source, source_generation) \
         VALUES \
