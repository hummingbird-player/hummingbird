...
use super::{
    OnSelectHandler,
    table_data::{Column, GridContext, RowResource, TableData, TableDragData},
};
use crate::ui::{
    app::Pool,
    components::{
    ...
    C: Column + 'static,
{
    ...
    row: RowResource<T::Identifier, T, T::RowState>,
    id: ElementId,
    ...
    C: Column + 'static,
{
    pub fn new(
        ...
        let row = AsyncResource::new(cx, key, load_row);
    ...
    C: Column + 'static,
{
    fn render(&mut self, _window: &mut Window, cx: &mut Context<'_, Self>) -> impl IntoElement {
        ...
        match loaded {
            Some((row_data, row_state)) => context(self.id.clone())
                .w_full()
                .h_full()
                .with(content)
                .try_menu_on_open(move |window, cx| {
                    row_data
                        .get_context_menu(window, cx, &menu_context, grid_context, &row_state)
                        .map(|(menu, overlay)| {
                            div()
                                .bg(menu_bg)
                                .child(menu)
